...
namespace POSales
{
    public partial class Discount : Form
    {
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                if (MessageBox.Show("Añadir descuento? Click yes para confirmar", stitle, MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    cn.Open();
                    cm = new SqlCommand("UPDATE Carrito SET disc_percent=@disc_percent WHERE id = @id", cn);                    
