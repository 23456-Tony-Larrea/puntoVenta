            ...
            try
            {
                ...
                else
                {
                    for(int i=0; i< cashier.dgvCash.Rows.Count; i++ )
                    {
                        ...
                        cn.Open();
                        cm = new SqlCommand("UPDATE Carrito SET status = 'Sold' WHERE id= '" + cashier.dgvCash.Rows[i].Cells[1].Value.ToString() + "'", cn);
                        cm.ExecuteNonQuery();
                        cn.Close();
                    }
                    ...
                    MessageBox.Show("Pago guardado correctamente!", "Pago", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    cashier.GetTranNo();
                    cashier.LoadCart();
                    this.Dispose();
                }
