...
namespace POSales
{
    public partial class Cashier : Form
    {
        ...
        private void btnClear_Click(object sender, EventArgs e)
        {
            ...
            {
                cn.Open();
                cm = new SqlCommand("Delete from Carrito where trasnno like'" + lblTranNo.Text + "'", cn);
                cm.ExecuteNonQuery();
                cn.Close();
                MessageBox.Show("Todos los elementos se han eliminado con éxito", "Remover el artículo", MessageBoxButtons.OK, MessageBoxIcon.Information);
                LoadCart();
            }
        }
        ...
        private void btnLogout_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("No se puede cerrar la sesión. Cancele la transacción.", "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
            if (MessageBox.Show("Salir de la aplicacion?", "Salir", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                this.Hide();
        ...
        public void LoadCart()
        {
            try
            {
                ...
                while (dr.Read())
                {
                    ...
                    dgvCash.Rows.Add(i, dr["id"].ToString(), dr["pcode"].ToString(), dr["pDesc"].ToString(), dr["price"].ToString(), dr["cantidad"].ToString(), dr["disc"].ToString(), double.Parse(dr["total"].ToString()).ToString("#,##0.00"));//
                    hascart = true;
                }
                dr.Close();
                cn.Close();
        ...
        public void GetTranNo()
        {
            try
            {
                ...
                string transno;
                cn.Open();
                cm = new SqlCommand("SELECT TOP 1 trasnno FROM Carrito WHERE trasnno LIKE '" + sdate + "%' ORDER BY id desc", cn);
                dr = cm.ExecuteReader();
                dr.Read();
                if (dr.HasRows)
                {
        ...
        private void txtBarcode_TextChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                    ...
                    int _qty;
                    cn.Open();
                    cm = new SqlCommand("SELECT * FROM Productos WHERE codigoBarras LIKE '" + txtBarcode.Text + "'", cn);
                    dr = cm.ExecuteReader();
                    dr.Read();
                    if(dr.HasRows)
                    {
                        qty = int.Parse(dr["cantidad"].ToString());
                        _pcode = dr["codigo"].ToString();
                        _price = double.Parse(dr["precio"].ToString());
        ...
        public void AddToCart(string _pcode, double _price,int _qty)
        {
            try
            {
                ...
                bool found = false;
                cn.Open();
                cm = new SqlCommand("Select * from Carrito Where trasnno = @trasnno and pcode = @pcode", cn);
                cm.Parameters.AddWithValue("@trasnno", lblTranNo.Text);
                ...
                {
                    id = dr["Id"].ToString();
                    cart_qty = int.Parse(dr["cantidad"].ToString());
                    found = true;
                }
                else found = false;
                dr.Close();
                ...
                if (found)
                {
                    ...
                    cn.Open();
                    cm = new SqlCommand("Update Carrito set cantidad = (cantidad + " + _qty + ")Where id= '" + id + "'", cn);
                    cm.ExecuteReader();
                    cn.Close();
        ...
        private void dgvCash_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (colName == "Delete")
            {
                if (MessageBox.Show("Eliminar este item", "Eliminar item", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    dbcon.ExecuteQuery("Delete from Carrito where id like'" + dgvCash.Rows[e.RowIndex].Cells[1].Value.ToString() + "'");
                    MessageBox.Show("Item removido satisfactoriamente", "Removido el item", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    LoadCart();
                }
            }
            else if (colName == "colAdd")
            {
                ...
                {
                    MessageBox.Show("La cantidad restante disponible es " + i + "!", "Agotado", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return;
                }
            }
            else if (colName == "colReduce")
            {
                int i = 0;
                cn.Open();
                cm = new SqlCommand("SELECT SUM(cantidad) as cantidad FROM Carrito WHERE pcode LIKE'" + dgvCash.Rows[e.RowIndex].Cells[2].Value.ToString() + "' GROUP BY pcode", cn);
                i = int.Parse(cm.ExecuteScalar().ToString());
                cn.Close();
                if (i > 1)
                {
                ...
                {
                    MessageBox.Show("La cantidad restante en el carrito es " + i + "!", "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return;
                }
            }
        }
